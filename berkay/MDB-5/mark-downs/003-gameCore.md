...
- Source: user interview + sketch reference (Flappy Bird–style layout: bird left, three gate sets with triangles and center diamond).
- Concept: Short Flappy Bird–style playable: tap to flap, scroll through three hand-placed gates matching the user’s layout sketch.
- Core loop: Wait on first tap → flap physics + horizontal scroll → clear three gates → win; hit a hazard or bounds → lose.
- First player action: Single tap anywhere; first tap both dismisses the tutorial hand and starts the run (with an immediate flap).
- Control method: Pointer/tap anywhere on screen; each tap while playing applies upward impulse (mobile-friendly).
- Camera/scene style: Orthographic side view (2.5D extruded shapes), fixed framing; world scroll moves obstacle groups left.
- Player entity: Small bird (circle body + beak) at fixed X; vertical motion only with gravity and flap.
- Targets/obstacles: Three pre-placed chunks — (1) top/bottom inward triangles, one gap; (2) small top spike, central diamond, small bottom spike, two vertical gaps; (3) same as (1). Layout follows the user’s reference sketch, not continuous random spawning.
- Challenge count/spawn rule: Exactly three gates (three chunks); no endless runner; obstacles do not respawn in this ad build.
- Collision model: Player uses a **circle** collider slightly smaller than the visible bird. Triangles and the diamond use **inset convex polygons** (no rectangular/AABB final test for those shapes); point-in-polygon and circle-to-segment distance for edges. Top/bottom world bounds also end the run.
- Object pooling: **Not used** — only three static obstacle groups that are repositioned on replay; no runtime spawn/destroy loop (recorded per booter exception for tiny static sets).
- Resources/progress values: Gates cleared 0–3 shown in HUD.
- Feedback: Gentle idle bob before start; HUD gate count; full-screen end overlay on win/lose.
- Tutorial guidance: Reusable **HandTutorial** with project `src/assets/hand-1.svg` (recognizable hand, fingertip-oriented anchor) prompting tap until the first click.
- Session length: Short skill moment (~15–25s typical) until three gates are passed or the player fails.
- Defaults chosen by AI and reasons: Scroll speed, gravity, and flap impulse tuned for a short ad; hazard polygons inset ~14–16% toward centroid for a forgiving mobile feel; win after all three gates passed because the sketch shows three obstacle sets; lose on any hazard or vertical bound for clear Flappy-style tension.
- Reference asset: User-supplied sketch copied to `src/assets/level-reference.png` for documentation; gameplay colliders were authored to match the described geometry, not sampled from the bitmap.
- Unresolved or deferred: Store URL for CTA is a placeholder (`example.com` / `mraid.open` when available); no background music; sound effects optional and not added in this pass.
