...
- Ending model: Win or lose (player can fail on hazards or bounds; win is achievable with skill).
- Win condition: All three gate chunks fully passed (trailing edge of each chunk passes the bird without a loss); HUD reaches 3/3.
- Lose condition: Circle bird overlaps any triangle/diamond polygon collider, or bird leaves the vertical play bounds.
- Timeout behavior: None; run ends only on win or lose.
- End-screen title: Win — `Nice flying!`; Lose — `Try again`.
- End-screen subtitle: Win — `You cleared every gate from the sketch.`; Lose — `Tap at the right moment to slip through the gaps.`
- CTA copy: `Play now` on both end states (conversion placeholder until a real store URL is provided).
- CTA trigger: Immediate when the user taps the primary button on the end panel; uses `mraid.open` when present, otherwise opens a placeholder URL in a new tab.
- Replay/reset behavior: Secondary `Play again` button on the end panel calls `resetRun()` — repositions chunks, resets HUD, restores idle state and HandTutorial until first tap.
- Gameplay cleanup: On win/lose, tutorial hand is destroyed; scrolling and flap input only apply while `playing`; pointer on end screen does not flap.
- Defaults chosen by AI and reasons: Dual outcome keeps Flappy-like tension; CTA still shown after loss to match typical playable ad flow; copy is short and readable on mobile.
- Unresolved or deferred: Final store/deep link and legal copy for the CTA; optional branded end card art.
