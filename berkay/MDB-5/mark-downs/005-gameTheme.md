...
- Theme: Flappy Bird–style minimal skill ad; geometry reads from the user’s black-and-white sketch (triangles + center diamond) translated into clean extruded shapes.
- Environment: Abstract light gray sky/backdrop (`#f0f0f0`), dark gray obstacles (`#2a2a2a`), bright bird for contrast.
- Mood/color palette: Casual arcade, high contrast for small screens; not a literal clone of any trademarked sprite set.
- Camera presentation: Fixed orthographic side view; readable silhouette shapes.
- Reference interpretation: Sketch = **layout and obstacle topology** (three sections, middle section hardest with diamond); polished into symmetric extruded meshes while preserving gap structure.
- Required assets: `src/assets/hand-1.svg` for HandTutorial (per MDB-5 README: clear hand, sleeve, shadow); user sketch stored as `src/assets/level-reference.png`.
- Placeholder strategy: No external network textures; all gameplay art is procedural Three.js geometry.
- Tutorial hand asset: `src/assets/hand-1.svg` at size ~132px, anchor `(0.22, 0.08)`, slight rotation so fingertip reads toward the tap pulse on center screen.
- Intro copy: None as separate overlay; idle state + animated hand implies “tap to start.”
- Tutorial copy: Hand gesture only until first tap (per user request).
- Win/lose copy: See `004-winLose.md` decision report for exact strings.
- CTA copy: `Play now` plus `Play again` for replay.
- Sound direction: None in this build (avoids autoplay issues; can add tap/flap/win/lose SFX later after first gesture).
- Constraints: Local assets only under `MDB-5/`; no dependency on other game folders for art.
- Defaults chosen by AI and reasons: Light neutral background matches sketch on white paper; dark hazards match ink lines; yellow bird improves visibility vs. gray field.
- Unresolved or deferred: Brand logo lockup, music, and final CTA destination.
