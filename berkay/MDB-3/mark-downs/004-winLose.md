...
- Ending model: Win or lose (skill-based).
- Win condition: Player **passes 10 obstacles** (clears 10 gaps).
- Lose condition: Collision with **obstacle, floor, or ceiling**.
- Timeout behavior: None specified; no forced timer end unless added later.
- End-screen title: Not specified—use short neutral lines (e.g. “You Win!” / “Game Over”) unless user provides copy.
- End-screen subtitle: Optional one line; deferred to theme/report.
- CTA copy: **Download Now** (both win and lose).
- CTA trigger: Shown on end screen after win or lose; **navigates to Google** (`https://www.google.com` or user’s final URL if swapped later).
- Replay/reset behavior: **Retry** button **only on lose** (full reset). No retry on win (CTA only unless user changes mind).
- Gameplay cleanup: Stop scrolling, input, and timers on end; hide tutorial hand.
- Defaults chosen by AI and reasons: Win/lose titles use simple placeholders because exact marketing copy was not provided.
- Unresolved or deferred: Final store/deeplink vs placeholder Google; exact button order/layout on end cards.
