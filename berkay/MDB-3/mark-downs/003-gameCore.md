...
- Concept: Flappy Bird–style playable; tap to flap through gaps.
- Core loop: Tutorial hand prompts tap; first tap starts the run. Player flaps to fly through **gaps between jagged top/bottom hazards** (and occasional diamond-style obstacles per reference); each **obstacle set passed** counts toward the win target.
- First player action: Tap anywhere (or on highlighted area per hand) to start and to flap.
- Control method: Tap / click for flap impulse; no input before first tap.
- Camera/scene style: Side-view 2D-style scroll (orthographic or equivalent); **portrait** layout (see theme report).
- Player entity: Bird (or equivalent flyer) with gravity + flap physics.
- Targets/obstacles: **Triangular / jagged / diamond** hazards (reference: `src/assets/layout-reference.png`), not rectangular pipes; **floor and ceiling** are lethal bounds.
- Challenge count/spawn rule: **Win after passing 10 obstacles** (10 obstacle sets / gaps cleared). Obstacles scroll in runner style until win or lose.
- Collision model: Lose on overlap with obstacle geometry, floor, or ceiling. Use **polygon or triangle-aware** colliders for non-rectangular shapes—**no oversized axis-aligned boxes** on spikes/diamonds; slightly **smaller than visible art** for forgiveness. Record chosen collider approach in code/decisions.
- Resources/progress values: Count of obstacles passed (0–10); optional light score UI.
- Feedback: Flap motion, pass feedback optional; clear stop on lose/win.
- Tutorial guidance: **Hand** indicates tap; game frozen or non-scoring until first tap.
- Session length: Until 10 passes (win) or collision (lose)—typically short ad length.
- Defaults chosen by AI and reasons: Spacing, scroll speed, and gap sizes not specified—will tune for readable mobile difficulty and ~15–30s typical win path unless adjusted in implementation.
- Unresolved or deferred: Exact scroll speed / gap height / gravity numbers; intro title copy; sound on/off detail; authored vs procedural obstacle sequences within reference visual language.
