...
- Source: user interview + reference sketch (chat/image description)
- Theme: Minimal **line-art / sketch** Flappy-like; sharp jagged hazards (stalactite/stalagmite spikes, floating diamond middle obstacle in one gap).
- Environment: Abstract side-scrolling corridor with **floor and ceiling** bounds (lose on contact).
- Mood/color palette: **Black-and-white** minimalist default matching sketch; can add subtle background if needed without breaking simplicity.
- Camera presentation: **Portrait** orientation (user-specified); side view, bird left-of-center feel per reference.
- Reference interpretation: Sketch shows **three obstacle layouts** as **style examples**; gameplay implements **10** passes with procedurally or patterned variants in that style—not only three total gaps.
- Required assets: Bird silhouette; jagged hazard meshes; UI for end screens; tutorial hand.
- Placeholder strategy: Simple geometric Three.js shapes acceptable if they match sketch language.
- Tutorial hand asset: **User chose built-in hand.** Use reusable `HandTutorial` with local **`src/assets/hand-1.svg`** (polished hand per README).
- Saved reference assets: User-provided sketch PNG was **not present** in `berkay/MDB-6/src/assets/` when logging; **deferred:** copy/link reference image into `src/assets/reference-flappy-sketch.png` when available.
- Orientation: **Portrait only** (primary layout).
- Intro copy: Optional light title (AI default: **"Flappy Gap"** or omit if busy—defer to implementation).
- Tutorial copy: Implicit via hand; optional text **"Tap to fly"** (AI default).
- Win/lose copy: See `004-winLose.md` decision report same date.
- CTA copy: **"Download now"** (see win/lose report).
- Sound direction: Optional short flap/hit/win SFX; **deferred**—add only if bundle size/time allows.
- Constraints: Match booter rules on hazard collision fidelity; no oversized rectangle hitboxes for triangular/diamond hazards.
- Defaults chosen by AI and reasons:
  - Monochrome sketch-like look aligns with user's reference drawing.
- User-confirmed choices:
  - Built-in `HandTutorial` + `src/assets/hand-1.svg` (explicit user request).
- Unresolved or deferred:
  - Re-save reference image into game `src/assets/`.
  - Optional sound and exact intro title text.
