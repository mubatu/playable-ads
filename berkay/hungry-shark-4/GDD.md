...
### Joystick Movement Requirements (Critical)

The shark's movement MUST map directly to the joystick direction.

Examples:

* Joystick right → shark moves right
* Joystick left → shark moves left
* Joystick up → shark moves upward
* Joystick down → shark moves downward
* Joystick up-right → shark moves diagonally up-right
* Joystick down-left → shark moves diagonally down-left

The joystick controls the shark's actual world movement direction, not only its rotation.

Movement vector must be calculated directly from joystick input:

movementDirection = normalize(joystickInput)

The shark's velocity must always be applied in this direction.

### Rotation Rules

The shark must always face the current movement direction.

Examples:

* Moving right → shark faces right
* Moving left → shark faces left
* Moving up → shark points upward
* Moving down-right → shark points down-right

Rotation should smoothly interpolate toward the movement direction.

### Camera Independence

Joystick input must be interpreted in world space and must NOT be affected by camera orientation, camera rotation, camera zoom, or camera movement.

Regardless of camera behavior:

* Right on joystick always means move right on screen.
* Left on joystick always means move left on screen.
* Up on joystick always means move toward the top of the screen.
* Down on joystick always means move toward the bottom of the screen.

The player should never experience inverted, rotated, mirrored, or camera-relative controls.

### Input Priority

The shark's position must be driven by joystick direction every frame.

If joystick input exists:

* Shark immediately starts moving in that direction.
* Previous momentum must not override the new direction.
* Inertia may slightly smooth acceleration, but it must never prevent the shark from following joystick direction.

Player input must always feel responsive and predictable.

### Implementation Constraint

The shark controller must use the joystick's X and Y values directly as the desired movement vector.

The shark must never continue moving in an old direction when the joystick is pointing somewhere else.

The shark must never orbit, drift, or move relative to camera axes.

The shark's movement direction and joystick direction must always match visually.
