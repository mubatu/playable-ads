...
- Theme: arcade Flappy Bird-style obstacle dodging.
- Environment: clean sky backdrop with side-view scrolling hazards.
- Mood/color palette: bright, high-contrast mobile arcade look.
- Camera presentation: portrait side-view orthographic composition.
- Reference interpretation: the user-provided sketch defines the bird silhouette, side-view layout, and sharp triangle/diamond/irregular obstacle style; it is treated as a gameplay/layout reference rather than final art.
- Required assets: local saved reference image and built-in tutorial hand SVG.
- Placeholder strategy: use simple Three.js shapes and CSS UI because no final art pack was provided.
- Tutorial hand asset: built-in hand requested; use local `src/assets/hand-1.svg` copied exactly from the default hand template.
- Saved reference assets: `src/assets/reference-flappy-sketch.png`.
- Orientation: portrait mobile.
- Intro copy: "Tap to Fly".
- Tutorial copy: "Tap to flap!".
- Win/lose copy: win title "Great Job!", lose title "Try Again!".
- CTA copy: "Play Now".
- Sound direction: lightweight interaction cues may be triggered after first input; no music required.
- Constraints: keep the ad fast-loading and self-contained with no external network assets.
- Defaults chosen by AI and reasons: polished simple shapes are used instead of preserving rough pencil lines because the sketch is a rough reference and readable mobile obstacles are more important for playable clarity.
- Unresolved or deferred: final brand logo, app icon, and CTA destination are not provided.
