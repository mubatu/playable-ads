...
- Concept: Flappy Bird-style playable ad where a small bird flies through sharp obstacle gaps.
- Core loop: the player taps/clicks to flap upward, gravity pulls the bird down, and obstacle sets scroll from right to left until the end state.
- First player action: click/tap the screen; the game begins on the first click.
- Control method: tap/click only, chosen for immediate mobile playable accessibility.
- Camera/scene style: portrait side-view orthographic 2.5D scene.
- Player entity: small flying bird inspired by the user's sketch.
- Targets/obstacles: 10 passable obstacle sets using spike, triangle, diamond, and irregular sharp shapes based on the reference sketch.
- Challenge count/spawn rule: win after passing exactly 10 obstacle sets; obstacle sets are spawned/reused as they scroll.
- Collision model: bird uses a forgiving circle collider; triangular, diamond, and irregular hazards use shape-aware polygon/circle collision with smaller forgiving gameplay colliders.
- Object pooling: repeated obstacle sets use pooling because runner-style obstacles are reused throughout the ad.
- Resources/progress values: progress counter tracks passed obstacle sets from 0/10.
- Feedback: flap motion, progress text, hit flash, and win/lose overlay feedback.
- Tutorial guidance: built-in hand shows the player to tap before the first interaction, then stops when play begins.
- Session length: about 15-25 seconds, driven by the 10-obstacle target.
- Defaults chosen by AI and reasons: obstacle colors, speed curve, and collision forgiveness will use bright readable arcade defaults because the user provided a rough shape/layout sketch but no color palette or tuning values.
- Unresolved or deferred: exact store URL is not provided, so CTA click will use a placeholder handler unless the user later supplies a destination.
