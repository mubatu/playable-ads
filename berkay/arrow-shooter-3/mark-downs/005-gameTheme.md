...
- Theme: Dark, clean, high-contrast 3D puzzle stage; bright colored arrows (GDD).
- Environment: Raised 3D board panel, conveyor rail loop, shooter row above top lane (GDD).
- Mood/color palette: Dark blues/greys background; saturated arrow colors from config palette.
- Camera presentation: Portrait-first 1080×1920 responsive; perspective camera per GDD defaults.
- Reference interpretation: No user reference images provided.
- Required assets: Built-in `src/assets/hand-1.svg` (exact template from this file).
- Placeholder strategy: Procedural Three.js geometry for board, arrows, conveyor, shooters, bullets.
- Tutorial hand asset: **Built-in hand** — `HandTutorial` with `src/assets/hand-1.svg` copied from Default Built-In Hand SVG template.
- Saved reference assets: None.
- Orientation: Portrait-first, responsive (GDD).
- Intro copy: None (tutorial hand on first valid arrow only).
- Tutorial copy: Hand gesture only; dismiss on first tap (GDD).
- Win/lose copy: `Board Cleared!` / `Conveyor Jammed!` (GDD).
- CTA copy: `Play Now` (GDD).
- Sound direction: SFX on tap, shoot, hit, win, lose **after first user touch**; no background music (user + AI default).
- Constraints: Mobile playable performance; no external art required for v1.
- Defaults chosen by AI and reasons:
  - Web Audio procedural SFX used if no `.wav` assets supplied — keeps repo small while meeting SFX requirement after first gesture.
