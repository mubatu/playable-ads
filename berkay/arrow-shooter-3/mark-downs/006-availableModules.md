...
This inventory is a guide, not a substitute for the implementation. Before using
a reusable module, inspect its actual source file under `reusables/` and follow
its current constructor, exports, lifecycle methods, and caveats.

Use `games/block-blast/` as the approved example for how a playable imports
modules from `reusables/`, organizes `src/js/` files, loads config, builds HUD,
and wires tutorial behavior. Treat it as an implementation-pattern reference,
not as gameplay or theme content to copy by default.
