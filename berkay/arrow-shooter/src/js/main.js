...
function createSharedResources() {
  return {
    cellGeometry: new THREE.PlaneGeometry(CONFIG.cellSize * 0.92, CONFIG.cellSize * 0.92),
    hitGeometry: new THREE.PlaneGeometry(CONFIG.cellSize, CONFIG.cellSize),
    arrowGeometry: createArrowGeometry(),
    arrowUnitGeometry: new THREE.PlaneGeometry(CONFIG.cellSize * 0.74, CONFIG.cellSize * 0.36),
    connectorGeometry: new THREE.PlaneGeometry(CONFIG.cellSize * 0.42, CONFIG.cellSize * 0.18),
    pipGeometry: new THREE.CircleGeometry(0.035, 16),
...
function addStage() {
  ...
  const panel = new THREE.Mesh(
    new THREE.PlaneGeometry(boardWidth + 0.45, boardHeight + 0.45),
    new THREE.MeshBasicMaterial({ color: 0x101a33, transparent: true, opacity: 0.94 })
  );
  panel.position.set(0, CONFIG.boardY, -0.05);
  scene.add(panel);

  const rim = new THREE.Mesh(
    new THREE.RingGeometry(2.95, 3.08, 96),
    new THREE.MeshBasicMaterial({ color: 0x2b456f, transparent: true, opacity: 0.55 })
  );
  rim.position.set(0, CONFIG.boardY - 0.15, -0.08);
  ...
  for (let row = 0; row < CONFIG.rows; row += 1) {
    for (let col = 0; col < CONFIG.columns; col += 1) {
      const cell = new THREE.Mesh(
        shared.cellGeometry,
        new THREE.MeshBasicMaterial({ color: (row + col) % 2 ? 0x172548 : 0x1b2d56 })
      );
...
function createArrow(data) {
  ...
  data.cells.forEach(([row, col], index) => {
    ...
    const unit = new THREE.Mesh(
      geometry,
      new THREE.MeshBasicMaterial({ color: color.fill })
    );
    const shadow = new THREE.Mesh(
      geometry,
      new THREE.MeshBasicMaterial({ color: color.dark, transparent: true, opacity: 0.5 })
    );
  ...
  for (let i = 0; i < data.cells.length - 1; i += 1) {
    ...
    const connector = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x9aa7ba, transparent: true, opacity: 0.9 })
    );
    const connectorShadow = new THREE.Mesh(
      shared.connectorGeometry,
      new THREE.MeshBasicMaterial({ color: 0x3a4558, transparent: true, opacity: 0.58 })
    );
...
function buildShooters() {
  ...
  specs.forEach((spec, index) => {
    ...
    const base = new THREE.Mesh(
      new THREE.BoxGeometry(0.92, 0.36, 0.08),
      new THREE.MeshBasicMaterial({ color: COLORS[spec.color].dark })
    );
    const barrel = new THREE.Mesh(
      new THREE.BoxGeometry(0.22, 0.48, 0.08),
      new THREE.MeshBasicMaterial({ color: COLORS[spec.color].fill })
    );
    shooter.base = base;
...
function buildConveyorTrack() {
  ...
  const rail = new THREE.LineLoop(
    new THREE.BufferGeometry().setFromPoints(points),
    new THREE.LineBasicMaterial({ color: 0x5d729d, transparent: true, opacity: 0.88 })
  );
  scene.add(rail);
  ...
  conveyorPath.points.forEach((point) => {
    const corner = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x92f7ff, transparent: true, opacity: 0.75 })
    );
    corner.position.set(point.x, point.y, 0.04);
    scene.add(corner);
  });
}
