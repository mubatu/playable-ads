...
- Concept: Flappy Bird–style side-scroller playable; triangular / spike obstacles inspired by the user’s layout sketch.
- Core loop: Tap to flap through gaps while the world scrolls; each cleared obstacle counts toward a target of 10; collision with hazards or bounds ends the run.
- First player action: Animated hand prompts a tap; the first pointer down starts the run and applies the first flap.
- Control method: Pointer / tap (or click) anywhere to flap while playing.
- Camera/scene style: Orthographic side view, portrait framing; simple 2.5D-style shapes in Three.js.
- Player entity: Bird (small circular collider for reliable mobile readability).
- Targets/obstacles: Scrolling triangular gate obstacles and occasional “diamond” middle beat with upper/lower paths (inferred from reference sketch for variety).
- Challenge count/spawn rule: Win after **10** obstacles passed; obstacles spawn at fixed horizontal spacing ahead of the player until the win condition is met.
- Resources/progress values: Obstacles cleared count (0–10), shown during play.
- Feedback: Motion, collision stop, and end overlays; optional simple motion cues on flap (implementation-level).
- Tutorial guidance: Reusable hand overlay until the first tap.
- Session length: Until 10 passes (win) or one collision / bound hit (lose).
- Defaults chosen by AI and reasons: Scroll speed, gravity, flap impulse, gap sizes, and obstacle spacing tuned for a short skill ad; diamond variant every third beat to mirror the reference middle section; **retry** immediately restarts in playing state without re-showing the hand (clearer loop after failure).
- Unresolved or deferred: None for core spec.
