...
- Theme: Minimal arcade Flappy-style; readable silhouettes over realism.
- Environment: Abstract side-scrolling corridor with floor/ceiling bounds and sharp triangular hazards.
- Mood/color palette: High-contrast, bright sky-like background, dark obstacles, warm accent on bird (friendly, mobile-readable).
- Camera presentation: Fixed orthographic side view; **portrait** orientation locked via resize logic and world aspect.
- Reference interpretation: Sketch used for **layout language** (triangular spikes, gated path, middle “split” hazard), not as final art; rough geometry kept readable and enlarged for small screens.
- Required assets: Hand tutorial image as a **small SVG inside this game folder** (no cross-folder asset copies).
- Placeholder strategy: Procedural Three.js primitives for bird and spikes; no external textures except the local hand SVG.
- Intro copy: Implicit (“tap to start” via hand only; no heavy title screen unless added later).
- Tutorial copy: Hand gesture only.
- Win/lose copy: See `004-winLose.md` decision report for titles; subtitles are short generic lines.
- CTA copy: **Download Now** (both outcomes).
- Sound direction: None requested; omitted to keep scope small (can add SFX after first gesture later).
- Constraints: Portrait-first; playable should stay lightweight for mobile WebGL.
- Defaults chosen by AI and reasons: Visual styling defaults where not specified; sketch informs obstacle shape language only.
