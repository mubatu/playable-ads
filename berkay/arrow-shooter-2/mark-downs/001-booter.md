...
The AI must edit one game folder only. The expected structure is:
...
The AI may inspect a small allowlist outside the given `game-name/` folder so it
can use the shared reusable modules correctly and follow the approved example.
It must not edit these external folders unless the user explicitly asks.

The AI may use:

- `game-name/README.md` if present,
- `game-name/GDD.md` if present,
...
- user-provided answers and assets that are explicitly placed inside
  `game-name/`,
- source files under the repo-level `reusables/` folder,
- files under `games/block-blast/` as the approved example for reusable-module
  usage, project structure, config loading, HUD/tutorial wiring, and general
  playable-ad organization.

The AI must not inspect unrelated game folders, example folders, or repo files
outside this allowlist. `games/block-blast/` is a reference for structure and
reusable integration, not a requirement to copy its puzzle mechanics, assets,
copy, or visual theme.
...
3. Read `GDD.md` if it exists in the current game folder. Treat it as the
   primary game-design source, and do not ask the user questions already
   answered there.
4. Read `002-questions.md` to understand how to interview the user.
5. Read the topic file that matches the next requirement area:
...
6. Read `006-availableModules.md` privately before planning implementation.
7. Inspect relevant source files under `reusables/` before importing a reusable
   module.
8. Inspect `games/block-blast/` when an implementation example is useful,
   especially for import paths, config loading, UI setup, tutorial wiring, and
   splitting code across `src/js/` modules.
...
`GDD.md`, `index.html`, `src/` folder, repo-level reusable modules, and the
approved `games/block-blast/` example.
...
- implementation choices grounded in the private reusable-module descriptions,
  actual reusable source, and the approved `games/block-blast/` example where
  relevant.
...
- Before asking questions, read `GDD.md` if it exists and use it to pre-fill
  known requirements. Ask only for missing, ambiguous, or conflicting details.
...
- Use `006-availableModules.md`, relevant `reusables/` source files, and
  `games/block-blast/` as internal implementation references. Never quote,
  summarize, or expose the private inventory directly to the user.
- Do not inspect other game folders, unrelated example folders, or unrelated
  project files.
- Prefer the reusable modules described in `006-availableModules.md` and verified
  in `reusables/` source when they fit the requested game.
...
- what the `GDD.md` specifies, if present,
...
- notes from relevant `reusables/` source or `games/block-blast/` patterns when
  those references affect the implementation plan.
