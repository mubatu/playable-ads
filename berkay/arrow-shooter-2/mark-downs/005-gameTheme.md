...
- Source: GDD.md, user interview, and Block Blast tutorial wiring reference.
- GDD specifies: clean, flat, vibrant 3D UI with a dark background so bright arrows pop, plus tactile arrow clicks, satisfying clearing particles, and screen shake.
- User explicitly requested: use the built-in local hand asset after asking for the Block Blast-style tutorial hand, use the GDD's portrait 1080x1920 direction, and leave CTA destination as a future store URL.
- Theme: clean hyper-casual arrow puzzle.
- Environment: dark minimal game stage with a bright board, top shooter row, and conveyor path around the grid.
- Mood/color palette: high-contrast dark background with vibrant arrow/shooter colors.
- Camera presentation: fixed portrait orthographic 2.5D, optimized for readable board state and tap targets.
- Reference interpretation: Block Blast is used only for tutorial wiring and reusable integration style, not for puzzle mechanics, copy, or art theme.
- Required assets: local tutorial hand SVG at `src/assets/hand-1.svg`; other visuals can be generated with simple geometry/CSS unless final brand assets are later provided.
- Placeholder strategy: use polished simple shapes for arrows, shooters, bullets, board cells, and particles; name/store any placeholder assets clearly if files are created.
- Tutorial hand asset: built-in local hand SVG copied exactly from this markdown's default template and used through the shared hand tutorial asset URL.
- Saved reference assets: none provided by user.
- Orientation: portrait.
- Intro copy: default title `Clear the Arrows`; default subtitle `Tap arrows that can escape. Match colors to destroy them.`
- Tutorial copy: default prompt `Tap a free arrow`.
- Win/lose copy: win `Board Cleared!`; lose `Conveyor Jammed!`.
- CTA copy: `Play Now`.
- Sound direction: light tap, hit, clear, invalid, win, and lose effects after first interaction; no background music by default.
- Constraints: keep assets self-contained inside `berkay/arrow-shooter/`; do not depend on the Block Blast hand image path because it points outside this game folder.
- Defaults chosen by AI and reasons: use the local built-in hand template because the requested Block Blast hand image is not inside the current game folder; use simple geometry instead of external assets because none were provided; use short ad copy because detailed brand copy was not provided.
- Unresolved or deferred: final logo, store URL, store-specific CTA policy, and custom sound files remain deferred.

## Decision Report - 2026-05-17 Visual Correction

- Source: user correction after reviewing the playable.
- User explicitly requested: remove the text saying `Clear every...` because it overlaps game objects, make arrow units more rectangular, add narrower grey connector squares between arrow units, show shooter bullet counts, and animate new shooters into shooter slots.
- UI correction: the mid-screen goal text was removed; only compact top HUD values and the bottom prompt remain.
- Arrow presentation correction: connected arrows use colored rectangular units and grey connector joints for readability.
- Shooter presentation correction: each shooter has a clear numeric bullet badge and queue replacement is shown as motion into the slot.
- Defaults chosen by AI and reasons: use generated canvas text badges for bullet counts because they remain crisp without adding external font/image assets.

## Decision Report - 2026-05-17 3D Conversion

- Source: user request to convert `berkay/arrow-shooter-2/` into 3D.
- Theme correction: the clean hyper-casual dark stage remains, but visual presentation now uses perspective depth, lighting, shadows, raised board cells, extruded arrows, tube conveyor rails, and 3D shooter barrels.
- Camera presentation: configurable perspective camera with an angled portrait puzzle view.
- Required assets: no new external art assets were added; 3D visuals are generated from Three.js geometry and the existing hand SVG remains unchanged.
- Placeholder strategy: use procedural 3D geometry and lit materials for fast loading and easy tuning.
- Defaults chosen by AI and reasons: preserve high-contrast bright colors from the 2D version because the GDD asks for vibrant arrows that pop against a dark background.
