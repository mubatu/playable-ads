...
import { getBlockTexture, getCellCenter } from './Board.js';
...
export function getRandomShape(colors) {
    ...
    return {
        cells: SHAPE_TEMPLATES[templateIndex],
...
export function createShapeGroup(shape, cellSize, gap, scale) {
    ...
    for (i = 0; i < shape.cells.length; i += 1) {
        var row = shape.cells[i][0];
        var col = shape.cells[i][1];
...
export function placeShapeOnGrid(state, shape, anchorRow, anchorCol) {
    ...
    for (i = 0; i < shape.cells.length; i += 1) {
        var cellRow = anchorRow + shape.cells[i][0];
        var cellCol = anchorCol + shape.cells[i][1];
...
export function updateGhostPreview(state, anchorRow, anchorCol, cells, color) {
    ...
    for (i = 0; i < cells.length && i < ghosts.length; i += 1) {
        ...
        ghosts[i].position.set(center.x, center.y, 0.12);
        ghosts[i].visible = true;

        if (color) {
            ghosts[i].material.color.set(color);
        } else {
            ghosts[i].material.color.set(0xffffff);
        }
    }
}
